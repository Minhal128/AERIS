...
def full_system_scan():
    ...
    neutralized_threats = set()  # Store removed threats
    ...
    while True:
        try:
            ...
            if "Threat" in result.stdout or "detected" in result.stdout:
                ...
                threat_id = f"{file_name} at {file_path}"
                
                # Check if the threat was already neutralized
                if threat_id in neutralized_threats:
                    print(Fore.YELLOW + f"[INFO] Skipping previously removed threat: {threat_id}" + Style.RESET_ALL)
                else:
                    print(Fore.RED + f"[ALERT] Malicious activity detected: {threat_id}! Neutralizing..." + Style.RESET_ALL)
                    subprocess.run(["powershell", "-Command", "Remove-MpThreat"], capture_output=True)
                    
                    # Verify if the file still exists
                    if os.path.exists(file_path):
                        print(Fore.RED + f"[ERROR] Failed to remove: {file_path}. Manual intervention required." + Style.RESET_ALL)
                    else:
                        print(Fore.GREEN + f"[SUCCESS] Threat {file_name} removed! System secure." + Style.RESET_ALL)
                        neutralized_threats.add(threat_id)  # Add to removed threats
